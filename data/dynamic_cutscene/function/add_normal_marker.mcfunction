...
execute as @e[type=marker,tag=cutscene_point,tag=positioned,limit=1,sort=nearest] run tag @s add temp_particles
execute as @e[type=marker,tag=cutscene_point,tag=positioned,limit=1,sort=nearest] run scoreboard players set @s dynamic_cutscene.particle_timer 150