...
# Calculate the position of the previous marker (the one we're interpolating FROM)
...
# Find and tag the previous marker
...
# Spawn the target marker (where we're interpolating TO) with special tag
...
# Get the interpolation parameters from storage and call create_interpolated_points
